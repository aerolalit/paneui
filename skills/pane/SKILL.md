...
later**. If you lost a URL, `pane session list` will not return it; instead,
use `pane session participant new` to mint a fresh URL on the still-alive
session.
...
### `pane session participant new|revoke` — manage URLs on a live session
...
Two primitives that together replace `pane session delete + pane session
...
```sh
pane session list                                          # find session_id + p_id
pane session participant new ses_abc123 | tee -a ~/.pane-sessions.jsonl
