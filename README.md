...
- **`@paneui/mcp`** — a thin stdio [MCP](https://modelcontextprotocol.io) server (binary `pane-mcp`) with **full parity with the CLI**. Point Claude Desktop, Cursor, or any MCP client at `npx @paneui/mcp` and Pane shows up as tools: the hot-path discrete tools (`create_pane`, `get_events`, `send_to_pane`, `update_pane`, `upgrade_pane`, `list_panes`, `delete_pane`, the record CRUD tools) plus consolidated `action`-based tools for templates, sharing, attachments, taste, keys, trash, feedback, agent identity, and `run_query` / `get_skill`. See the [package README](packages/mcp/README.md) for the full tool list and client config snippets.
