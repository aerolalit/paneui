...
URL — *that is the redirect*. The CLI prints it; `pane watch` surfaces it. Nobody logs
...
1. Creates a pane with the bundled tutorial artifact (+ its event schema).
2. Opens the URL in the browser (or prints it if no browser is available).
3. **Runs a tiny built-in agent loop in the same terminal process** that watches the pane,
...
Event schema (the contract `pane demo` registers for the pane):

| Event type | Emitted by | Payload | Purpose |
|---|---|---|---|
| `demo.start` | human | `{}` | Scene 1 "Show me how" — navigation into the model |
| `demo.hello` | human | `{}` | Scene 3 click — the round-trip proof |
...
button, *"Show me how →"* → emits `demo.start`.
...
that command is acting as your agent right now. It's watching this pane."*

**Scene 3 — First emit (the proof beat).** A single button: *"Click me."* → emits `demo.hello`.
The demo-agent loop receives it, prints it to the terminal, and replies `demo.advance{scene:3,
...
log your agent can read."* Show the CLI equivalent inline: `pane show <id>`.
...
- **New top-level command** `pane demo` (sits alongside the existing `pane create`,
  `pane watch`, `pane show`, and `pane send` verbs; reuses the same config/profile resolution).
...
- **Cleanup.** The demo pane is created with a short TTL so the relay's sweeper reclaims it;
  the command also best-effort `DELETE`s the pane on exit.
...
- **DECIDED — OPEN-2 resolved as *split*.** Scene 1's *"Show me how"* emits `demo.start`
  (navigation); Scene 3's *"Click me"* emits `demo.hello` (the round-trip proof). Cleaner
  schema, and each scene's event reads as exactly what it is.
- **DECIDED — Landing page uses simulated mode.** The same artifact is mounted on the landing
  ...
  live relay pane is provisioned for anonymous visitors → no abuse surface, no cost, no auth.
...
- **DECIDED — CSS keyframes + Web Animations API, not GSAP.** The artifact runs under the
  ...
  scene transitions don't justify bundling GSAP into the artifact; native CSS/WAAPI keeps the
...
2. **`pane demo`** — pane create + the watch/react loop + terminal echo + clean exit.
