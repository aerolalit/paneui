...
Pane's `/b/<token>` endpoint serves an attachment's bytes without an agent API
key or a participant pane token. **The URL token IS the credential.**
...
From inside a pane iframe, a direct `<img src="/b/<token>">` (or
the equivalent `<iframe>`, `fetch`, etc.) will fail. Two of the
defences in the table below collaborate to block it:

- The shell serves the pane under a strict CSP whose `img-src` is
  `'self' data:` — third-party-looking image URLs are refused even if
  they resolve to the same relay.
- Attachment responses set `Cross-Origin-Resource-Policy: same-origin`, so
  even when the pane iframe shares the relay's origin, a
  speculative cross-origin embed from a sandboxed frame is denied.

The supported path for in-pane rendering is the page-side SDK:

```js
// inside the pane's HTML
const bytes = await window.pane.downloadBlob(attachmentId);
const objectUrl = URL.createObjectURL(new Blob([bytes]));
document.getElementById("hero").src = objectUrl;
```

`window.pane.downloadBlob(id)` is a postMessage RPC into the shell;
the bytes come back as an `ArrayBuffer`, which you can wrap in a
browser `Blob` + object URL and feed to `<img>` / `<video>` / `<a download>`.

For this RPC to succeed, the relay must consider the attachment
**referenced from this pane** — either it is `scope=pane` for
this pane, or its id appears at a `format: pane-attachment-id` site in
the pane's `input_data` (walked against `input_schema`) or in an
event payload (walked against the event-type schema). Agent-scope or
template-scope attachments that the agent hasn't explicitly surfaced via
those channels return `attachment_ref_not_accessible` even when the agent
that minted the pane owns them.

**Rule of thumb:**

| Where the attachment is consumed | Use |
|---|---|
| Inside the pane (iframe) | `window.pane.downloadBlob(id)` |
...
| Scope-bound TTL (30d template / pane-TTL / 24h agent) | bounded exposure window | leaked URL stops working at expiry |
...
| `Referrer-Policy: no-referrer` on every attachment response | block cross-origin leak | the next-page navigation can't leak the URL |
...
| `Content-Security-Policy: default-src 'none'; sandbox; frame-ancestors 'none'` + `X-Frame-Options: DENY` | block framing AND defang any rendered document | `frame-ancestors 'none'` closes the same-site framing gap CORP alone leaves (an inline image attachment can otherwise be embedded by a same-site page even though CORP blocks cross-origin reads); `default-src 'none'; sandbox` neutralises scripts/forms/same-origin if the bytes are ever rendered as a document. Applied to all attachment + icon download responses. |
...
```
              Is the attachment sensitive (private docs, personal photos)?
                 ┌────────────────┴────────────────┐
                NO                                YES
                 │                                 │
      ...
      NO                    YES          NO                YES
       │                     │            │                 │
   Use the                Pane-scope   `--once` token    Agent-scope
   authenticated          attachment   (5-min TTL)       attachment
   GET /v1/attachments/:id with        + encryption-at-  + agent-scope
   from agent code        pane-scope   rest opt-in.      token (24h, can
   only — never           token that   Re-mint per       be re-minted
   /b/<token>             dies with    download.         any time)
                          the pane.
```

### When to use `--once`

- The attachment is sensitive and you're sharing it with exactly one human.
...
- You want the human to be able to refresh / re-open the page that
  embeds the attachment (`<img src=/b/...>`).
- The attachment is benign (a public-ish UI icon, a chart).
...
This wraps each attachment with a per-attachment random DEK encrypted under
`PANE_SECRET_KEY` (AES-256-GCM). An attacker who reads the raw attachment
files from disk cannot decrypt them without the master key.

What this does NOT defend against:
- Relay compromise. If the attacker has the master key, every attachment is
  readable. (Customer-managed keys are v2.)
- Master-key loss. Without `PANE_SECRET_KEY`, encrypted attachments are
  unrecoverable. **Back the key up.**

## How to audit a token's usage

```sh
# Show all tokens for an attachment, including audit metadata.
pane attachment show <attachment_id>
...
## Participant uploads (`POST /s/:participantToken/attachments`)

The agent-side upload path is `POST /v1/attachments` with an agent API key. To
...
multipart uploads on `POST /s/:participantToken/attachments` authenticated by
...
fetch and returns an `AttachmentRef` to the iframe via postMessage. The agent
then receives the AttachmentRef as part of a normal `pane.emit(...)` event
payload and can download the bytes via the existing
`/v1/attachments/:id` route or by minting a `/b/<token>` URL.
...
- **Scope pinning.** Scope is FORCED to `pane`. Even if the multipart
  body carries `scope=agent` or `scope=template`, the route ignores
  those values. A human cannot mint long-lived agent-scope attachments through
  a participant token, and cannot reach a template they don't own.
  Pane-scope attachments cascade-delete with the pane, which bounds the
  blast radius of a leaked participant token: when the agent ends the
  pane, the human's uploads go with it.
- **Quota accounting.** Uploads count against the OWNING AGENT's quota
  (`MAX_BLOBS_PER_AGENT_BYTES`), not the participant. The attachment row's
  `ownerId` is `pane.agentId`. This means a hostile / runaway
  participant can DoS the agent's quota; agents should keep an eye on
  `pane attachment list` and/or set conservative per-pane caps with
  `MAX_BLOBS_PER_PANE_BYTES`.
- **Pipeline parity.** The participant route runs the EXACT same pipeline
  as `POST /v1/attachments` — MIME sniff, polyglot defense via sharp, EXIF
  strip, envelope encryption-at-rest, scan webhook. The shared
  implementation lives in `packages/relay/src/attachments/upload-pipeline.ts`
...
Response: an `AttachmentRef` (same shape `POST /v1/attachments` returns).
...
| 410  | `gone` | the pane is closed or expired |
| 413  | `attachment_size_exceeded` | upload > `MAX_BLOB_BYTES` |
...
## Participant downloads (`GET /s/:participantToken/attachments/:attachment_id`)

The symmetric counterpart to `POST /s/:participantToken/attachments`. Lets the
pane running inside the rendered iframe lazy-fetch attachment bytes by id,
...
The pane API is `await window.pane.downloadBlob(attachment_id)` — see
...
- **Authz.** The requested `attachment_id` MUST be referenced from this
  pane — either in the pane's initial `inputData` (validated
  against the template version's `inputSchema`) or in any event in the
  pane (validated against the pane's `eventSchema`). A participant
  cannot use their token to enumerate attachments that aren't already part of
  this pane's transcript. Cross-pane probing returns the same
  opaque 404 as a nonexistent attachment.
- **Defense in depth.** After the ref check, the route also verifies the
  attachment's owning agent matches the pane's owning agent and the attachment is
  not soft-deleted. The walker's set should already be agent-scoped (the
  write-side check in `core/events.ts` only persists attachment refs that
...
- **Decryption.** The route runs the EXACT same decrypt pipeline as the
  agent-side `GET /v1/attachments/:id` when `BLOB_ENCRYPT_AT_REST` is on. Both
  routes share the same `encrypt.parseEnvelope` + `decryptBlob` calls,
  so the two cannot drift on envelope semantics. The `/b/<token>`
  capability-URL pane and the participant-token `/s/.../attachments/:id`
  pane BOTH return plaintext; both have e2e coverage that diffs the
  on-disk ciphertext against the response bytes
  (`bridge/attachment-download-bridge.e2e.test.ts` for the participant route,
  `routes/attachments.e2e.test.ts` "envelope encryption-at-rest" cases for `/b/<token>`).
...
GET /s/<participantToken>/attachments/<attachment_id>
  → 200 with the decrypted bytes
    Content-Type:        <attachment.mime>      (sniffed at upload time)
    Content-Length:      <attachment.size>      (plaintext)
...
| HTTP | code | meaning |
|------|------|---------|
| 400  | `invalid_request` | malformed `attachment_id` shape |
| 401  | `participant_token_invalid` | malformed / unknown / revoked token |
| 404  | `attachment_ref_not_accessible` | attachment_id is not referenced from this pane, was soft-deleted, or never existed |
| 410  | `gone` | the pane is closed or expired |
