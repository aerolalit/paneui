...
| 4 | [`phase-4-mcp-ttl-deploy.md`](./phase-4-mcp-ttl-deploy.md) | The TTL sweeper, the MCP server (the first client wrapper) and its tools (`create_pane_session`, `await_pane_result`, `get_pane_state`), the Dockerfile / single-container deploy, the `claudeclaw` dogfood, and the hosted-lite demo. |
...
3. **Interfaces**: the exact contracts the code must satisfy. Prisma models, endpoint request/response shapes, the shim API, the MCP tool signatures, env vars.
