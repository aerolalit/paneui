...
The agent generates and ships a UI artifact, optionally with a per-session event schema. Humans and agents are peers connected to a single session. Every interaction (a human click, an agent reply) is an event. State is what you get by replaying events. The relay transports and validates; it does not interpret. An artifact with no event schema is view-only — a report or dashboard the human only reads.
...
3. The agent declares the session's event schema up front (or omits it for a view-only artifact). The relay validates writes against it.
...
- `type` must exist in the session's event schema. 422 otherwise. A view-only session (no event schema) rejects every `page`/`agent` write `422 unknown_event_type`.
...
The agent ships this at session creation — or omits it. Shape:
...
- **The event schema is optional.** An artifact that omits it is **view-only**: a report, dashboard, or chart the human only reads. The session declares an empty, strictly-enforced event vocabulary — every `page`/`agent` emit is rejected `422 unknown_event_type`. System events still flow, and a view-only artifact may still carry an `input_schema` (a reusable report template seeded per session with `input_data`).
...
| `event_schema`   | JSON, nullable. The event vocabulary for sessions on this version. `null` = a view-only version (no event vocabulary). |
...
3. Lookup `type` in `session.event_schema`. 422 if missing. A view-only session (`event_schema` is `null`) has no vocabulary — every `page`/`agent` write is rejected `422 unknown_event_type`.
