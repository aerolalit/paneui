...
- **Records — per-pane mutable record collections.** Third first-class data shape alongside events and attachments (epic #287). The event log is wrong for "thousands of comments where only the latest value matters" — read cost, write contention, and wire cost all break at scale. Records solve those: a mutable per-pane collection (one row = one current value), declared via a JSON Schema 2020-12 document with one namespaced extension (`x-pane-collections`). Standards-first by design so agents authoring templates only learn one extra keyword. Shipped: schema (#288), validator (#289), compat (#290), writer (#291), routes (#292), config + sweeper (#293), WS wire + replay (#294/#295), bridge auto-subscribe (#296), CLI (#297), page-side read API (#298 phase 1). Mutator runtime API + `eventSchema` standards alignment are queued follow-ups.
