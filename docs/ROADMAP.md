...
- [ ] Four-table Prisma schema (`agents`, `sessions`, `participants`, `events`) per `SPEC.md`. `prisma migrate`, TTL cleanup job. `events.id` is BIGINT; the `?since=` cursor is opaque in the API.
...
- [ ] DB-backed bearer-token auth for agent → relay: keys in `agents` (`sha256` + display prefix + `revoked_at`). `POST /register` behind `REGISTRATION_SECRET`. Per-identity participant tokens issued at session create.
...
- [ ] MCP server: `create_pane_session(artifact, schema)` returns the human URL(s); `await_pane_result(session_id, terminal_event_type)` blocks until that event or the TTL.
