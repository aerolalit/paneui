...
-- Dedup unique index — lives under Prisma's conventional name, not the
-- `name:` attribute from @@unique (that attribute is just the TS query
-- name in Prisma 7, not the DB constraint name).
ALTER INDEX "surfaces_template_version_id_owner_human_id_context_key_key" RENAME TO "panes_template_version_id_owner_human_id_context_key_key";
