// Envelope encryption-at-rest for attachment bytes.
//
// Per-attachment random 32-byte data-encryption-key (DEK). The DEK is wrapped
...
//     who reads the raw attachment bytes from object storage cannot decrypt
//     them without the master key.
//   * Self-host filesystem snooping. Same property: the bytes on disk
//     are ciphertext.
//
// What this does NOT defend against:
//   * Relay compromise. If the attacker has the master key, every attachment
 ...
 * `encryption_envelope` column of the attachment row. Fields are base64-encoded
 * binary; `v` is the envelope version and is the seam for future schemes
 * (CMK, key rotation, larger ciphers, etc.).
 */
export interface AttachmentEncryptionEnvelope {
...
export function encryptBlob(
  plaintext: Buffer,
  master: Buffer,
): { ciphertext: Buffer; envelope: AttachmentEncryptionEnvelope } {
...
export function parseEnvelope(raw: string): AttachmentEncryptionEnvelope {
  const json = Buffer.from(raw, "base64").toString("utf8");
  const parsed = JSON.parse(json) as Partial<AttachmentEncryptionEnvelope>;
  if (parsed.v !== 1 || !parsed.wd || !parsed.di || !parsed.dt) {
    throw new Error("malformed AttachmentEncryptionEnvelope");
  }
  return parsed as AttachmentEncryptionEnvelope;
}
