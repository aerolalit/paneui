...
// real-Azure variant (attachment-conformance-real-azure.yml) overrides the
...
runConformanceSuite({
  ...
  caps: {
    presign: true,
    presignScopedToSingleKey: true, // SAS sr=b is per-attachment
  },
  ...
  setup: async () => {
    ...
    return {
      store: s,
      nextKey: () => `attachment_${randomBytes(8).toString("hex")}`,
    };
  },
});
