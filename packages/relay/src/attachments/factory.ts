// AttachmentStore factory — selects the backend from config + lazy-loads its SDK.
//
...
/**
 * Build the configured AttachmentStore. Call once at relay startup; the returned
 ...
 */
export async function makeBlobStore(config: Config): Promise<AttachmentStore> {
...
function isModuleNotFound(e: unknown): boolean {
  ...
  if (code === "ERR_MODULE_NOT_FOUND" || code === "MODULE_NOT_FOUND") {
    ...
    return /@azure\/(storage-attachment|identity)/.test(msg);
  }
  return false;
}
