...
import { AttachmentSizeExceededError, FilesystemBlobStore } from "./index.js";

let dir: string;

beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "attachment-fs-test-"));
});
...
describe("FilesystemBlobStore round trip", () => {
  it("put → head → get → delete round-trips identical bytes", async () => {
    ...
    const info = await store.put("attachment_x", streamOf(payload), {
      mime: "image/jpeg",
      maxBytes: 1_000_000,
    });
    ...
    const head = await store.head("attachment_x");
    expect(head).not.toBeNull();
    expect(head!.size).toBe(info.size);
    expect(head!.sha256).toBe(info.sha256);

    const stream = await store.get("attachment_x");
    ...
    await store.delete("attachment_x");
    expect(await store.head("attachment_x")).toBeNull();
    expect(await store.get("attachment_x")).toBeNull();
  });

  it("get + head return null for an unknown key", async () => {
    const store = new FilesystemBlobStore({ dir });
    await store.init();
    expect(await store.get("attachment_nope")).toBeNull();
    expect(await store.head("attachment_nope")).toBeNull();
  });

  it("delete is idempotent (no error on a missing key)", async () => {
    const store = new FilesystemBlobStore({ dir });
    await store.init();
    await store.delete("attachment_nothing-here");
    await store.delete("attachment_nothing-here");
  });
  ...
  it("written files have mode 0600", async () => {
    const store = new FilesystemBlobStore({ dir });
    await store.init();
    await store.put("attachment_mode", streamOf(Buffer.from("x")), {
      mime: "application/octet-stream",
      maxBytes: 100,
    });
    const st = statSync(join(dir, "attachment_mode"));
...
describe("FilesystemBlobStore size cap", () => {
  it("rejects mid-stream when bytes exceed maxBytes (no partial file persists)", async () => {
    ...
    await expect(
      store.put("attachment_big", Readable.from(big), {
        mime: "application/octet-stream",
        maxBytes: 10 * 1024,
      }),
    ...
    expect(await store.head("attachment_big")).toBeNull();
    expect(await store.get("attachment_big")).toBeNull();
  });
  ...
  it("accepts a payload exactly equal to maxBytes", async () => {
    ...
    const info = await store.put("attachment_exact", Readable.from(exact), {
      mime: "application/octet-stream",
      maxBytes: 1024,
    });
...
describe("FilesystemBlobStore integrity", () => {
  it("head() returns null when the sidecar is corrupt", async () => {
    const store = new FilesystemBlobStore({ dir });
    await store.init();
    await store.put("attachment_corrupt", streamOf(Buffer.from("hi")), {
      mime: "application/octet-stream",
      maxBytes: 100,
    });
    // Smear the sidecar with garbage.
    await writeFile(join(dir, "attachment_corrupt.meta.json"), "not json {{{");
    const got = await readFile(
      join(dir, "attachment_corrupt.meta.json"),
      "utf8",
    );
    expect(got).toBe("not json {{{"); // sanity: the write landed
    expect(await store.head("attachment_corrupt")).toBeNull();
  });
});
...
describe("FilesystemBlobStore sidecar field naming (ciphertext_sha256)", () => {
  it("new writes use 'ciphertext_sha256' on disk (and only that key)", async () => {
    ...
    const raw = await readFile(join(dir, "attachment_new.meta.json"), "utf8");
  ...
  it("head() round-trips a freshly written attachment via ciphertext_sha256", async () => {
    const store = new FilesystemBlobStore({ dir });
    await store.init();

    const payload = Buffer.from("round-trip-payload");
    const info = await store.put("attachment_rt", streamOf(payload), {
      mime: "image/png",
      maxBytes: 100,
    });
    const head = await store.head("attachment_rt");
    expect(head).not.toBeNull();
    expect(head!.size).toBe(info.size);
    expect(head!.sha256).toBe(info.sha256);
  ...
  it("head() still reads legacy sidecars that use the bare 'sha256' key", async () => {
    ...
    const info = await store.put("attachment_legacy", streamOf(payload), {
      mime: "image/jpeg",
      maxBytes: 100,
    });
    const legacy = {
    ...
    await writeFile(
      join(dir, "attachment_legacy.meta.json"),
      JSON.stringify(legacy),
    );

    const head = await store.head("attachment_legacy");
    expect(head).not.toBeNull();
    expect(head!.size).toBe(info.size);
    expect(head!.sha256).toBe(info.sha256);
  ...
  it("head() returns null when neither sidecar hash key is present", async () => {
    ...
    await store.put("attachment_nohash", streamOf(payload), {
      mime: "application/octet-stream",
      maxBytes: 100,
    });
    // Overwrite the sidecar with a JSON object that has size but no hash.
    await writeFile(
      join(dir, "attachment_nohash.meta.json"),
      JSON.stringify({
        size: payload.length,
        mime: "application/octet-stream",
      }),
    );
    expect(await store.head("attachment_nohash")).toBeNull();
  });
});
