...
//
// One battery of behavioural tests parametrised over a `AttachmentStore`
...
import { AttachmentIntegrityError, type AttachmentStore } from "./store.js";
...
export interface ConformanceContext {
  store: AttachmentStore & Partial<PresignCapable>;
...
/**
 * Run the shared backend-conformance battery against one `AttachmentStore`
 ...
 *       setup: () => ({ store: makeFsStore(), nextKey: () => `attachment_${randHex()}` }),
 *     });
 *   });
 *
...
export function runConformanceSuite(opts: {
  ...
  describe(`[#154] ${backendName} — backend conformance`, () => {
    ...
    // PUT a attachment, GET it, verify byte-for-byte equality. Smallest possible
    ...
    //
    // The invariant: no half-written / "Frankenstein" attachment. Three legal
    // outcomes:
    ...
    it("[8] concurrent put() to same key leaves a consistent state", async (testCtx) => {
      await skipUnavailable(
        async () => {
          ...
          // Outcome (a) or (b): a attachment exists. head() must claim one of the
    ...
    presignDescribe(`[#154] ${backendName} — presigned PUT cases`, () => {
      ...
      // AttachmentIntegrityError (size mismatch) and removes the bytes.
      // --------------------------------------------------------------------
      ...
      // test is meaningful at the AttachmentStore layer; the stronger
      ...
      // `sr=b` (single-attachment scope) in the SAS. If `sr=c` (container)
        ...
        it("PUT to key Y with a SAS minted for key X is rejected", async (testCtx) => {
          await skipUnavailable(
            async () => {
              ...
              // Rewrite the URL path: replace the X attachment name with Y.
              // Azure SAS URLs look like `https://.../container/<attachment>?sas=...`
