// End-to-end tests for the /v1/templates routes — reusable, versioned
// templates owned by an agent. Exercises create / version / patch / search /
// get-by-id / get-by-slug / get-version, ownership isolation, and the
// per-agent / per-template caps. DB engine follows DATABASE_URL.
...
function createArtifact(
  ...
  return req("POST", "/v1/templates", apiKey, {
    name: "PR Review",
    slug: "pr-review",
...
describe("/v1/templates", () => {
  ...
  it("creates a named template and returns template_id + version 1", async () => {
  ...
  // View-only template: created with no event_schema. The version persists a
  // null event_schema and GET serializes it as `event_schema: null`.
  it("creates a view-only template when event_schema is omitted", async () => {
    const apiKey = await seedAgent();
    ...
    const ver = await req(
      "GET",
      `/v1/templates/${template_id}/versions/1`,
      apiKey,
    );
  ...
  it("appends a view-only version when event_schema is omitted", async () => {
    ...
    const res = await req(
      "POST",
      `/v1/templates/${created.template_id}/versions`,
      apiKey,
      {
    ...
    const ver = await req(
      "GET",
      `/v1/templates/${created.template_id}/versions/2`,
      apiKey,
    );
    expect(((await ver.json()) as { event_schema: unknown }).event_schema).toBe(
      null,
  ...
  it("appends a new version and bumps latest_version", async () => {
    ...
    const get = await req(
      "GET",
      `/v1/templates/${created.template_id}`,
      apiKey,
    );
    const full = (await get.json()) as {
  ...
  it("appends a version addressed by slug, not just id", async () => {
    ...
    const res = await req("POST", "/v1/templates/by-slug/versions", apiKey, {
      source: "<html><body>v2</body></html>",
      type: "html-inline",
      event_schema: eventSchema,
    });
  ...
  it("patches head metadata addressed by slug", async () => {
    ...
    const res = await req("PATCH", "/v1/templates/patch-by-slug", apiKey, {
  ...
  it("searches by q over name/description/tags", async () => {
    ...
    const all = await req("GET", "/v1/templates", apiKey);
    expect(
      ((await all.json()) as { templates: unknown[] }).templates,
    ).toHaveLength(2);

    const hit = await req("GET", "/v1/templates?q=feedback", apiKey);
    const hitBody = (await hit.json()) as {
      templates: { slug: string }[];
    };
    expect(hitBody.templates).toHaveLength(1);
    expect(hitBody.templates[0]!.slug).toBe("survey");
    // Lean response — no source attachment.
    expect(hitBody.templates[0]).not.toHaveProperty("source");
  });

  it("gets an template by id and by slug", async () => {
    const apiKey = await seedAgent();
    const created = (await (await createArtifact(apiKey)).json()) as {
      template_id: string;
    };
    ...
    const bySlug = await req("GET", "/v1/templates/pr-review", apiKey);
  ...
  it("gets a specific version's full content", async () => {
    ...
    const res = await req(
      "GET",
      `/v1/templates/${created.template_id}/versions/1`,
      apiKey,
    );
    expect(res.status).toBe(200);
  ...
  it("returns 404 for another agent's template", async () => {
    const owner = await seedAgent();
    const other = await seedAgent();
    const created = (await (await createArtifact(owner)).json()) as {
      template_id: string;
    };
    const res = await req("GET", `/v1/templates/${created.template_id}`, other);
...
describe("DELETE /v1/templates/:id (#137)", () => {
  ...
  it("also accepts a slug", async () => {
    const apiKey = await seedAgent();
    await createArtifact(apiKey);
    const del = await req("DELETE", "/v1/templates/pr-review", apiKey);
    expect(del.status).toBe(204);
  });
  ...
  it("returns 404 artifact_not_found on the second delete (not idempotent at the row level)", async () => {
    ...
    expect(
      (await req("DELETE", `/v1/templates/${created.template_id}`, apiKey))
        .status,
    ).toBe(204);
