...
//
// The mark is the "Pane" robot: a coral (#D97757) rounded-square tile with an
// all-white chat-bubble face (two eyes) and lens circle. It is identical to the
// marketing site mark (site/favicon.svg) and the home-screen / PWA install
// icons (src/app-icon.ts), so the logo is the SAME on every surface — browser
// tab favicon, in-app header + sidebar, the pane viewer shell, and the
// installed app icon.
//
 ...
 * Self-contained — it carries its own coral tile background — so it renders as
 ...
 */
export const BRAND_MARK_SVG_BODY = `<rect width="100" height="100" rx="22" fill="#D97757"/>
  <circle cx="62" cy="58" r="17.5" fill="#ffffff"/>
