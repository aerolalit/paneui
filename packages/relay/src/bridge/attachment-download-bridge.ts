...
//   * Authz is "the requested attachment_id must be REFERENCED FROM THIS SESSION."
//     A participant can only download attachments the owning agent has explicitly
//     referenced in the surface — either in the surface's initial
//     `inputData` (validated against the template version's `inputSchema`)
//     or in any event in the surface (validated against the surface's
//     `eventSchema`). Cross-surface probing returns 404, never 403 — an
//     attacker probing must not be able to distinguish "attachment exists, but
//     not in your surface" from "attachment does not exist at all."
//   * Defense in depth after the ref check: the attachment row's owning agent
//     must match the surface's owning agent and the row must not be soft-
...
// hotspot we can materialise referenced attachment_ids into a `SurfaceBlobRef`
// join table at write time (the only place refs are introduced into a
// surface is the writeEvent + surface-create paths, both already running
...
//
// Errors:
//   401 participant_token_invalid — token malformed / unknown / revoked / surface gone
//   400 invalid_request           — attachment_id of malformed shape
//   404 attachment_ref_not_accessible   — attachment_id not referenced from this surface
//   410 gone                      — surface closed/expired
blobDownloadBridge.get(
  ...
  async (c) => {
    ...
    if (!BLOB_ID_RX.test(attachmentId)) {
      ...
      // from "valid shape, not accessible from this surface" which is 404.
      throw errors.invalidRequest(
    ...
    // Resolve participant -> surface -> agent. Identical to the upload bridge.
    ...
    // Authz: a attachment is accessible to a participant of THIS surface if EITHER
    //   (a) the attachment is referenced from this surface's inputData or events —
    //       i.e. the agent put it on the wire here (PR #164's walker; same
    //       check the write side uses), OR
    //   (b) the attachment is scope=surface AND its surfaceId matches this surface.
    //
    // Branch (b) is needed because:
    //   - participant uploads (POST /s/:tok/attachments, PR #165) pin scope=surface
    //     and surfaceId=surface.id at write time. Without (b), the participant
    ...
    //   - agent-uploaded surface-scoped attachments (scope=surface, surfaceId set
    //     when the agent uploads) are by construction part of this surface's
    //     surface even if no event references them yet.
    //
    // Branch (b) does NOT loosen the model — surface-scope means surface-
    ...
    if (row && row.scope === "surface" && row.surfaceId === surface.id) {
      accessible = true;
    } else {
      const referenced = await collectSessionBlobRefs(prisma, surface);
      if (referenced.has(attachmentId)) accessible = true;
    }

    if (!accessible) {
      // Opaque 404 — never reveal whether the id exists but lives in another
      // surface vs. doesn't exist at all. Reusing PR #164's
      // `attachment_ref_not_accessible` code keeps the surface consistent.
    ...
    // cross-tenant leak. Verify the row exists, is owned by THIS surface's
    // agent, is `ready`, and not soft-deleted.
    if (
      !row ||
      row.ownerId !== surface.agentId ||
      row.status === "deleted" ||
      row.deletedAt !== null
    ) {
...
// ---------------------------------------------------------------------------
// Collect every attachment_id referenced from a surface — either in its initial
...
async function collectSessionBlobRefs(
  prisma: PrismaClient,
  surface: SurfaceForRefs,
): Promise<Set<string>> {
  const acc = new Set<string>();

  // 1) inputData against inputSchema (when both exist).
  const inputSchema = surface.templateVersion.inputSchema as object | null;
  if (inputSchema && surface.inputData !== null) {
    for (const id of collectBlobRefs(inputSchema, surface.inputData)) {
      acc.add(id);
    }
  }
