// End-to-end test for `POST /s/:participantToken/blobs` — follow-up C of #156.
//
// Mirrors the surface tests in src/http/routes/blobs.e2e.test.ts so the human-
...
beforeAll(async () => {
  blobDir = mkdtempSync(join(tmpdir(), "blob-bridge-e2e-"));
...
interface UploadOpts {
  declaredMime?: string;
  filename?: string;
  /** Extra form fields the route should IGNORE — used to verify forced session scope. */
...
async function upload(
  ...
  fd.set("file", blob, opts.filename ?? "human.jpg");
  ...
  return app.fetch(
    new Request(`http://t/s/${token}/blobs`, {
      method: "POST",
      body: fd,
    }),
  );
...
describe("POST /s/:participantToken/blobs — happy path", () => {
  beforeEach(async () => {
    await testDb.truncateAll(prisma);
  });

  it("uploads a JPEG and returns a BlobRef pinned to scope='session'", async () => {
    const { token, agentId, sessionId } = await seedSession();
    ...
    // The Blob row's `ownerId` is the AGENT that owns the session, not the
    // participant — human uploads count against the agent's footprint.
    const row = await prisma.blob.findUnique({ where: { id: json.blob_id } });
  ...
  it("forces scope=session even when the client tries to send scope=agent", async () => {
...
describe("POST /s/:participantToken/blobs — auth / token validation", () => {
  beforeEach(async () => {
    await testDb.truncateAll(prisma);
  });
  ...
  it("rejects uploads to a closed session with 410 gone", async () => {
    const { token, sessionId } = await seedSession();
  ...
  it("rejects uploads to an expired session with 410 gone", async () => {
    const { token, sessionId } = await seedSession();
    await prisma.session.update({
      where: { id: sessionId },
...
describe("POST /s/:participantToken/blobs — rejection paths", () => {
  beforeEach(async () => {
    await testDb.truncateAll(prisma);
  });
...
describe("POST /s/:participantToken/blobs — polyglot defense", () => {
  beforeEach(async () => {
    await testDb.truncateAll(prisma);
  });

  it("strips HTML appended after a JPEG (same defense as POST /v1/blobs)", async () => {
    const { token } = await seedSession();
