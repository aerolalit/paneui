...
//
// Entities cascade naturally via Prisma's onDelete: Cascade — surface
// hard-delete takes events / participants / record_collections /
// surface_records with it. Attachments are special: we must also purge
...
// `sweepExpiredSurfaces` in `index.ts`): after surface hard-deletes, any
// anonymous template (name + slug both null) with zero remaining surface
 ...
 *
 * Order matters: children first (surfaces, attachments) so cascades don't
 * fight us. Anonymous-template orphan cleanup runs after surfaces.
 */
export async function sweepHardDeletable(
  ...
  // ---- 1. SURFACES ------------------------------------------------------
  // Surfaces own their child entities via Prisma cascades, so deleting a
  // surface ripples through events / participants / record_collections /
  // surface_records / feedback / attachments-of-this-surface automatically.
  const surfacesSwept = await sweepSurfaces(prisma, freeCutoff, paidCutoff);

  // ---- 2. ANONYMOUS TEMPLATE ORPHANS ------------------------------------
  // After surface deletes, find anonymous templates (name AND slug both
  // null) with zero remaining surface references and reclaim them. Skip
  ...
  // ---- 5. AGENTS --------------------------------------------------------
  // Soft-deleted agents past retention. Agents cascade to surfaces,
...
async function sweepSurfaces(
  prisma: PrismaClient,
  freeCutoff: Date,
  paidCutoff: Date | null,
): Promise<number> {
  // Candidate query: every soft-deleted surface. We filter by tier-aware
  ...
  const candidates = await prisma.surface.findMany({
...
async function reclaimAnonymousTemplateOrphans(
  ...
  // surface references is unreachable — no agent can re-instantiate it.
  // `versions.none.surfaces.some` reads as "no version has any surface".
