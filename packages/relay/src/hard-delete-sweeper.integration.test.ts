...
async function seedSoftDeletedSurface(opts: {
  agentId: string;
  ownerHumanId?: string;
  deletedDaysAgo: number;
}): Promise<string> {
  // seedSurfaceRow doesn't honor ownerHumanId; set it explicitly after seed
...
describe("sweepHardDeletable — tier-aware retention", () => {
  it("free row past 30d is reclaimed", async () => {
    ...
    const surfaceId = await seedSoftDeletedSurface({
      agentId,
    ...
    expect(
      await prisma.surface.findUnique({ where: { id: surfaceId } }),
    ).toBeNull();
  });
  ...
  it("surface with no owner human uses HARD_RETENTION_DAYS_FREE", async () => {
...
describe("sweepHardDeletable — cascade + orphan + audit", () => {
  it("surface hard-delete cascades to events + record_collections + participants", async () => {
    ...
    expect(await prisma.event.count({ where: { surfaceId } })).toBe(0);
    expect(await prisma.recordCollection.count({ where: { surfaceId } })).toBe(
      0,
    );
    expect(await prisma.participant.count({ where: { surfaceId } })).toBe(0);
  });

  it("anonymous template with zero remaining surfaces is reclaimed", async () => {
  ...
  it("named template survives even after its surfaces are reclaimed", async () => {
  ...
  it("appends one deletion_log row per hard-delete with phase + reason + owner anchors", async () => {
    ...
    const logs = await prisma.deletionLog.findMany({
      where: { entityType: "surface", entityId: surfaceId },
    });
