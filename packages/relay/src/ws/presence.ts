...
//
// Tracks, per surface, the set of currently-open WebSocket connections and
...
//   surfaceId -> (connId -> kind). Correct and sufficient for one replica;
//   behaviour is byte-for-byte as before.
//
//   REDIS ON (multi-replica) — a Redis hash per surface, `pane:presence:<id>`,
...
//     fields counted forever. We defend with a per-surface-hash TTL that the
...
//     nothing refreshes its surfaces' hashes and Redis evicts them once the
//     TTL lapses — the cluster self-heals within one TTL window. A surface
...
//     (Per-surface rather than per-connection TTL keeps it one key per
//     surface; the cost is that one dead connId can linger until the whole
//     surface hash expires — acceptable because the count is advisory and a
...
// In-process backend: surfaceId -> (connId -> kind).
const connections = new Map<string, Map<string, ConnKind>>();

// Redis key for a surface's presence hash.
const PRESENCE_PREFIX = "pane:presence:";
function presenceKey(surfaceId: string): string {
  return PRESENCE_PREFIX + surfaceId;
}

// Per-surface presence-hash TTL. Must comfortably exceed the WS heartbeat
// interval (30s, see ws/handler.ts HEARTBEAT_INTERVAL_MS) so a live surface is
// always refreshed before it lapses, while still evicting a dead replica's
// surfaces reasonably promptly. 90s = 3 missed heartbeats.
...
export async function addConnection(
  ...
  if (!redisEnabled()) {
    let perSession = connections.get(surfaceId);
    if (!perSession) {
      perSession = new Map();
      connections.set(surfaceId, perSession);
    }
    perSession.set(connId, kind);
    return;
  }
  try {
    const key = presenceKey(surfaceId);
...
export async function removeConnection(
  ...
  if (!redisEnabled()) {
    const perSession = connections.get(surfaceId);
    if (!perSession) return;
    perSession.delete(connId);
    if (perSession.size === 0) connections.delete(surfaceId);
    return;
  }
  try {
    await redisPub().hdel(presenceKey(surfaceId), connId);
  } catch (err) {
...
/**
 * Refresh a surface's presence-hash TTL. Called by the WS heartbeat for every
 * surface that still has a live socket on this replica, so a genuinely-active
 * surface never expires. No-op when Redis is off (the in-process Map has no
 * TTL — a crashed single replica takes its whole process with it).
 */
export async function refreshSession(surfaceId: string): Promise<void> {
  if (!redisEnabled()) return;
  try {
    await redisPub().pexpire(presenceKey(surfaceId), PRESENCE_TTL_MS);
  } catch (err) {
...
// Read every connId -> kind entry for a surface, from whichever backend.
async function readSession(surfaceId: string): Promise<Map<string, ConnKind>> {
  if (!redisEnabled()) {
    return connections.get(surfaceId) ?? new Map();
  }
  try {
    const hash = await redisPub().hgetall(presenceKey(surfaceId));
...
// How many agent-kind sockets are open on this surface right now.
export async function agentCount(surfaceId: string): Promise<number> {
  const perSession = await readSession(surfaceId);
  let n = 0;
  for (const kind of perSession.values()) if (kind === "agent") n++;
  return n;
}

// How many sockets (of any kind) are open on this surface right now. Used to
// enforce the per-surface WebSocket connection cap at upgrade time.
export async function connectionCount(surfaceId: string): Promise<number> {
  const perSession = await readSession(surfaceId);
  return perSession.size;
}

// How many human-kind sockets are open on this surface right now.
export async function humanCount(surfaceId: string): Promise<number> {
  const perSession = await readSession(surfaceId);
  let n = 0;
...
// Total open WebSocket connections across every surface, optionally filtered
...
export async function totalConnections(kind?: ConnKind): Promise<number> {
  ...
  try {
    ...
    for (const key of keys) {
      const surfaceId = key.slice(PRESENCE_PREFIX.length);
      const perSession = await readSession(surfaceId);
      if (kind === undefined) {
        n += perSession.size;
      } else {
