...
//
// Storage:
//   * Each blob is stored at `<storageKey>` in the configured container.
...
//
// TOCTOU defence: after the client posts /v1/blobs/<id>/confirm, the relay
// calls head() and verifies `size` + the sha256 metadata stamp match what
// was committed at presign time. A mismatch rejects the blob and removes
// the bytes.
...
export interface AzureBlobStoreOpts {
   ...
   *  - `{ kind: "accountUrl", url: "https://<acct>.blob.core.windows.net" }`
...
export class AzureBlobStore implements BlobStore {
  ...
  async put(
    ...
    // it into block-blob blocks under the hood; we tap the stream to count
    ...
    try {
      ...
      if (e instanceof BlobSizeExceededError) {
        // Don't leak a partial block-blob — best-effort delete.
    ...
    // re-downloading the blob to recompute.
  ...
  async head(key: string): Promise<BlobObjectInfo | null> {
    ...
    try {
      ...
      if (!sha256) {
        ...
        // complete the setMetadata step, or someone wrote a blob outside
        // the relay. Treat as integrity failure (caller's TOCTOU check
        // will refuse the blob).
        return null;
      }
      return {
        size,
  ...
  // -------------------------------------------------------------------------
  // Presigned PUT support (presign + verify). Not on the BlobStore interface
  ...
  async presignPut(opts: {
    ...
    const blob = this.container.getBlockBlobClient(opts.key);
   ...
   * verified info on success; throws `BlobIntegrityError` on mismatch.
   *
   * The caller (route layer) is responsible for stamping the sha256 we
   * computed at presign time into the blob's metadata BEFORE this verifies
  ...
  async confirmPresigned(
    ...
    const blob = this.container.getBlockBlobClient(key);
    const props = await blob.getProperties().catch((e) => {
      if (isNotFound(e)) return null;
      throw e;
    });
    if (!props) {
    ...
    // Compute the actual sha256 by streaming the blob back. Slightly
    ...
    // Stamp the verified sha256 + content-type as blob metadata so future
    // head() calls can return them without recomputing.
    await blob.setMetadata({ sha256: actualSha });
    await blob.setHTTPHeaders({ blobContentType: expected.mime });
...
function extractAccountName(accountUrl: string): string {
  // `https://<account>.blob.core.windows.net[/...]` — the host's first label.
