// BlobStore — backend-agnostic interface for the bytes side of a blob.
//
...
//     Lands in PR #3 of the blob stack — interface is shaped to fit ahead of
...
// key, e.g. `blob_<id>`) — backends are responsible for mapping that into
// whatever native location their storage system uses (a path under
// BLOB_STORE_FS_DIR, a blob-name in an Azure container, an S3 object key).

import type { Readable } from "node:stream";

/**
 * Metadata the BlobStore observes about a stored object — populated by HEAD
...
/**
 * Options for the relay-proxied write path (used by `POST /v1/blobs` multipart
 * in v0.1.0). Streams the body to the backend; computes sha256 and total size
 * as it goes; rejects with `blob_size_exceeded` if total exceeds `maxBytes`.
 */
...
export interface BlobStore {
   ...
   *
   * Throws `BlobSizeExceededError` if the streamed body would exceed
   * `opts.maxBytes` (cuts the stream and discards any partial write).
   */
  put(key: string, body: Readable, opts: WriteOpts): Promise<BlobObjectInfo>;
   ...
   * no-op success. The blob row's `deletedAt` is updated separately by the
...
/**
 * Thrown by `put()` when the streamed body exceeds the per-blob byte cap.
 * Distinct error type so the route layer can map it to the `blob_size_exceeded`
...
export class BlobSizeExceededError extends Error {
  constructor(
    ...
    super(
      `blob upload exceeded ${maxBytes} bytes (saw ≥ ${observed} bytes before cutting the stream)`,
    );
...
export class BlobIntegrityError extends Error {
  constructor(
    ...
    super(
      `blob bytes don't match commitment: expected size=${expected.size} sha256=${expected.sha256}, observed size=${observed.size} sha256=${observed.sha256}`,
    );
