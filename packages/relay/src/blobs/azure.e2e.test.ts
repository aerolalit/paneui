...
import { BlobIntegrityError, BlobSizeExceededError } from "./store.js";
...
beforeAll(async () => {
  ...
  try {
    ...
    // Tighter probe: actually round-trip a small blob to make sure the
...
const KEY = () => `blob_${randomBytes(8).toString("hex")}`;
...
describe("AzureBlobStore — round trip", () => {
  ...
  itAvailable("get + head return null for unknown keys", async () => {
    expect(await store.head("blob_no_such_key")).toBeNull();
    expect(await store.get("blob_no_such_key")).toBeNull();
  });

  itAvailable("delete is idempotent", async () => {
    await store.delete("blob_does_not_exist_either");
    await store.delete("blob_does_not_exist_either");
  });
});
...
describe("AzureBlobStore — size cap", () => {
  itAvailable(
    "throws BlobSizeExceededError when payload exceeds maxBytes",
    async () => {
      const key = KEY();
