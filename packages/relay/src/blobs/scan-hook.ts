...
// configured URL with a JSON body containing the blob's metadata and an
// HMAC-SHA256 signature in the `X-Pane-Scan-Signature` header. The scanner
// returns:
//   * 200 with `{ verdict: "clean" }` — blob → ready
//   * 200 with `{ verdict: "infected", reason?: string }` — blob → failed,
//     bytes deleted from storage
//   * anything else (timeout, non-2xx, missing/wrong signature on the
//     response) — fail-closed: blob → failed, bytes deleted
//
...
//   * Fetch the blob bytes if it needs to inspect content. The relay
//     supplies a fresh, single-use blob token URL in the request body so
...
export interface ScanRequestBody {
  ...
  sha256: string;
  /**
   * A `/b/<token>` URL the scanner can GET to fetch the blob bytes. Issued
...
/**
 * POST the blob's metadata to the configured BLOB_SCAN_HOOK and return the
